<?php
    ...
    */
    ...
    $fields = ["nome", "posicao", "escritorio", "salario"];
   ...
   if($searchValue != ''){
      $i=0; 
      $tamanho=sizeof($fields)-1;

      //monto a estrutura sql
      foreach($fields as $field){
         
         if($i==0){
            $sql=" AND ($field LIKE :$field";
         }else{
            $sql.=" OR $field LIKE :$field";
         }            
      ...
      //jogo sql para a variavel searchQuery
      $searchQuery = $sql;
      /**
       * monta o código
       * $searchArray = array('nome'=>"%$searchValue%",         
       */
      $searchArray = array();
        foreach($fields as $field){
